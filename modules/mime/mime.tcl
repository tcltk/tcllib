...
proc mime::qp_encode {string {encoded_word 0}} {
    ...
    regsub -all {[\x22=\x5B-\x5D\x7B\x7D]} \
	$string {[format =%02X [scan "\\&" %c]]} string

    # Replace outlying characters that would be munged by EBCDIC gateways.

    regsub -all {[\x00-\x08\x0B-\x1E\x21\x23\x24\x40\x5E\x60\x7C\x7E-\xFF]} \
	$string {[format =%02X [scan {&} %c]]} string
