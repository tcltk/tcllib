...
static void MDTestSuite ()
{
  ...
  $handle ccommand md5tcc_final {dummy ip objc objv} {
      ...
      MD5Final(mp);
      size = sizeof mp->digest;
      Tcl_SetObjResult(ip, Tcl_NewByteArrayObj(mp->digest,size));
...
if {[info exists pkgtest] && $pkgtest} {
  ...
  proc md5tcc_try {} {
    foreach {msg expected} {
      ...
      puts "testing: ::md5::md5tcc \"$msg\""
      binary scan [::md5::md5tcc $msg] H* computed
    ...
    foreach len {10 50 100 500 1000 5000 10000} {
      ...
      puts "input length $len: [time {md5tcc $blanks} 1000]"
    }
  }
